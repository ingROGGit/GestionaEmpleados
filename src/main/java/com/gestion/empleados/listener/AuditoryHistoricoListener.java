...
public class AuditoryHistoricoListener {
	private final AuditoryRepository auditoryRepository;

	@PrePersist
	private void prePersist(HistoricoExtSINAVIDEntity auditory) {
		AuditoryEntity auditoryEntity = this.getAuditory("INSERT", auditory);
		this.auditoryRepository.save(auditoryEntity);
	}

	@PreUpdate
	private void preUpdate(HistoricoExtSINAVIDEntity auditory) {
		AuditoryEntity auditoryEntity = this.getAuditory("UPDATE", auditory);
		this.auditoryRepository.save(auditoryEntity);
	}

	@PreRemove
	private void preRemove(HistoricoExtSINAVIDEntity auditory) {
		AuditoryEntity auditoryEntity = this.getAuditory("DELETE", auditory);
		this.auditoryRepository.save(auditoryEntity);
	}

	private AuditoryEntity getAuditory(String operacion, HistoricoExtSINAVIDEntity auditory) {
		...
		auditoryEntity.setName(auditory.getFileName());
